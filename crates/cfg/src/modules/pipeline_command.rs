...
pub fn default_command(id: i32) -> Option<PipelineCommand> {
    match id {
        ...
        20 => Some(PipelineCommand::PixelClassifier(
            PixelClassifierSettings::default(),
        )),
        21 => Some(PipelineCommand::RankFilter(RankFilterSettings::default())),
        22 => Some(PipelineCommand::RollingBall(RollingBallSettings::default())),
        23 => Some(PipelineCommand::SaveImage(SaveImageSettings::default())),
        24 => Some(PipelineCommand::Stardist(StardistSettings::default())),
        25 => Some(PipelineCommand::StructureTensor(
            StructureTensorSettings::default(),
        )),
        26 => Some(PipelineCommand::Threshold(ThresholdSettings::default())),
        27 => Some(PipelineCommand::TransformObjects(
            TransformObjectsSettings::default(),
        )),
        28 => Some(PipelineCommand::UNet(UNetSettings::default())),
        29 => Some(PipelineCommand::Voronoi(VoronoiSettings::default())),
        30 => Some(PipelineCommand::Watershed(WatershedSettings::default())),
        31 => Some(PipelineCommand::WeightedDeviation(
...
impl PipelineCommand {
    pub fn name(&self) -> &str {
        match self {
            ...
            Self::PixelClassifier(_) => "PixelClassifier",
    ...
    pub fn to_parameters(&self) -> Vec<ParameterDef> {
        match self {
            ...
            Self::PixelClassifier(_s) => [vec![ParameterDef { name: "model_path".to_string(), display_name: "Model Path".to_string(), description: "Path to a TorchScript-exported Cellpose model (`torch.jit.script`/`torch.jit.trace`).".to_string(), value: _s.model_path.display().to_string(), param_type: ParamType::FilePath, options: vec!["evamodel".to_string()], min: 0.0f32, max: 0.0f32, step: 1.0000f32, groups: vec![] }], vec![ParameterDef { name: "segmentation_mapping".to_string(), display_name: "Segmentation Mapping".to_string(), description: "Segmentation mapping list.\n\nMaps the segmentation class from the pixel classifier output to a segmentation class of the project".to_string(), value: String::new(), param_type: ParamType::Group, options: vec![], min: 0.0f32, max: 0.0f32, step: 1.0000f32, groups: _s.segmentation_mapping.iter().map(|__item| [vec![ParameterDef { name: "segmentation_class".to_string(), display_name: "Segmentation Class".to_string(), description: "Segmentation class from the classifier model".to_string(), value: format!("{}", __item.segmentation_class.as_u32()), param_type: ParamType::SegClass, options: vec![], min: 0.0f32, max: 0.0f32, step: 1.0000f32, groups: vec![] }], vec![ParameterDef { name: "object_class_id".to_string(), display_name: "Object Class Id".to_string(), description: "The classification ID assigned to pixels falling the segmentation class from the model".to_string(), value: format!("{}", __item.object_class_id.as_u32()), param_type: ParamType::SegClass, options: vec![], min: 0.0f32, max: 0.0f32, step: 1.0000f32, groups: vec![] }]].concat()).collect() }]].concat(),
