...
use crate::core_types::{MemoryId, PixelUnits, SizeUnits};
...
impl PipelineCommand {
    ...
    pub fn to_parameters(&self) -> Vec<ParameterDef> {
        match self {
            ...
            Self::ColorFilterCommand(_s) => [vec![ParameterDef { name: "range.min_h".to_string(), display_name: "Min H".to_string(), description: "Minimum Hue angle in degrees [0.0, 360.0].".to_string(), value: format!("{}", _s.range.min_h), param_type: ParamType::Number, options: vec![], min: 0.0f32, max: 0.0f32, step: 1.0000f32, groups: vec![] }], vec![ParameterDef { name: "range.max_h".to_string(), display_name: "Max. Hue".to_string(), description: "Maximum Hue angle in degrees [0.0, 360.0].".to_string(), value: format!("{}", _s.range.max_h), param_type: ParamType::Spinner, options: vec![], min: 0.0f32, max: 360.0f32, step: 1.0000f32, groups: vec![] }], vec![ParameterDef { name: "range.min_s".to_string(), display_name: "Min S".to_string(), description: "Minimum Saturation normalized [0.0, 1.0].".to_string(), value: format!("{}", _s.range.min_s), param_type: ParamType::Number, options: vec![], min: 0.0f32, max: 0.0f32, step: 1.0000f32, groups: vec![] }], vec![ParameterDef { name: "range.max_s".to_string(), display_name: "Max. Sat.".to_string(), description: "Maximum Saturation normalized [0.0, 1.0].".to_string(), value: format!("{}", _s.range.max_s), param_type: ParamType::Spinner, options: vec![], min: 0.0f32, max: 1.0f32, step: 0.1000f32, groups: vec![] }], vec![ParameterDef { name: "range.min_v".to_string(), display_name: "Min V".to_string(), description: "Minimum Value (Brightness) normalized [0.0, 1.0].".to_string(), value: format!("{}", _s.range.min_v), param_type: ParamType::Number, options: vec![], min: 0.0f32, max: 0.0f32, step: 1.0000f32, groups: vec![] }], vec![ParameterDef { name: "range.max_v".to_string(), display_name: "Max. Brightness".to_string(), description: "Maximum Value (Brightness) normalized [0.0, 1.0].".to_string(), value: format!("{}", _s.range.max_v), param_type: ParamType::Spinner, options: vec![], min: 0.0f32, max: 1.0f32, step: 0.1000f32, groups: vec![] }]].concat(),
    ...
    pub fn to_summary(&self) -> String {
        match self {
            ...
            Self::ColorFilterCommand(s) => format!(
                "Max. Hue: {} · Max. Sat.: {} · Max. Brightness: {}",
                format!("{:.3}", s.range.max_h),
                format!("{:.3}", s.range.max_s),
                format!("{:.3}", s.range.max_v)
            ),
            ...
            Self::ObjectMath(s) => format!(
                "Operation: {}",
                match s.operation {
                    ObjectObjectMathObjectSetOperationSettings::And => "And".to_string(),
                    ObjectObjectMathObjectSetOperationSettings::Or => "Or".to_string(),
                    ObjectObjectMathObjectSetOperationSettings::Xor => "Xor".to_string(),
                    ObjectObjectMathObjectSetOperationSettings::Subtract => "Subtract".to_string(),
                }
            ),
            ...
            Self::TransformObjects(s) => format!(
                "Function: {}",
                match s.function {
                    ObjectTransformObjectsTransformFunctionSettings::Scale { .. } =>
                        "Scale".to_string(),
                    ...
                    ObjectTransformObjectsTransformFunctionSettings::FittingEllipse { .. } =>
                        "Fitting Ellipse".to_string(),
                    ObjectTransformObjectsTransformFunctionSettings::Expand { .. } =>
                        "Expand".to_string(),
                    ObjectTransformObjectsTransformFunctionSettings::Shrink { .. } =>
                        "Shrink".to_string(),
                }
            ),
        ...
        match self {
            Self::AiObjectClassifier(s) => {
                ...
                if param_name == "match_handling" {
                    s.match_handling = match value { "Add class on match" => ObjectAiObjectClassifierAiClassifyMatchHandlingSettings::AddOutputClassIfMatch, "Reclassify on match" => ObjectAiObjectClassifierAiClassifyMatchHandlingSettings::ReclassifyIfMatch, _ => (s.match_handling).clone() };
                }
            }
            Self::Blur(s) => {
            ...
            Self::ClassifyObjects(s) => {
                ...
                if param_name == "match_handling" {
                    s.match_handling = match value { "Add class on match" => ObjectClassifyObjectsClassifyMatchHandlingSettings::AddOutputClassIfMatch, "Add class on mismatch" => ObjectClassifyObjectsClassifyMatchHandlingSettings::AddOutputClassIfNotMatch, "Remove class on match" => ObjectClassifyObjectsClassifyMatchHandlingSettings::RemoveInputClassIfMatch, "Remove class on mismatch" => ObjectClassifyObjectsClassifyMatchHandlingSettings::RemoveInputClassIfNotMatch, "Remove output class on match" => ObjectClassifyObjectsClassifyMatchHandlingSettings::RemoveOutputClassIfMatch, "Remove output class on mismatch" => ObjectClassifyObjectsClassifyMatchHandlingSettings::RemoveOutputClassIfNotMatch, "Remove objects matching criteria" => ObjectClassifyObjectsClassifyMatchHandlingSettings::RemoveAllClassesIfMatch, "Keep objects matching criteria" => ObjectClassifyObjectsClassifyMatchHandlingSettings::RemoveAllClassesIfNotMatch, "Reclassify on match" => ObjectClassifyObjectsClassifyMatchHandlingSettings::ReclassifyIfMatch, "Reclassify on mismatch" => ObjectClassifyObjectsClassifyMatchHandlingSettings::ReclassifyIfNotMatch, _ => (s.match_handling).clone() };
                }
            ...
            Self::Colocalization(s) => {
                ...
                if param_name == "multiplicity" {
                    s.multiplicity = match value {
                        "No multi coloc (1:1)" => {
                            ObjectColocObjectsColocMultiplicitySettings::OneToOne
                        }
                        "Allow multi coloc" => {
                            ObjectColocObjectsColocMultiplicitySettings::ManyToMany
                        }
                        "Multi coloc only for selected" => {
                            ObjectColocObjectsColocMultiplicitySettings::MultiFor(vec![])
                        }
                        _ => (s.multiplicity).clone(),
                    };
                }
                if let ObjectColocObjectsColocMultiplicitySettings::MultiFor(ref mut __inner) =
                    s.multiplicity
                {
            ...
            Self::ObjectMath(s) => {
                if param_name == "operation" {
                    s.operation = match value {
                        "And" => ObjectObjectMathObjectSetOperationSettings::And,
                        "Or" => ObjectObjectMathObjectSetOperationSettings::Or,
                        "Xor" => ObjectObjectMathObjectSetOperationSettings::Xor,
                        "Subtract" => ObjectObjectMathObjectSetOperationSettings::Subtract,
                        _ => (s.operation).clone(),
                    };
                }
                    ...
                    if let (Some(_i), Some(nested_name)) = (_p.next(), _p.next()) {
                        if let Ok(_idx) = _i.parse::<usize>() {
                            if let Some(item) = s.thresholds.get_mut(_idx) {
                                ...
                                if let SegmentationThresholdThresholdMethodSettings::RobustBackground { ref mut lower_outlier_fraction, ref mut upper_outlier_fraction, ref mut averaging_method, ref mut deviations_above_average,  } = item.method { if nested_name == "method.lower_outlier_fraction" { if let Ok(v) = value.parse::<f32>() { *lower_outlier_fraction = v; } } if nested_name == "method.upper_outlier_fraction" { if let Ok(v) = value.parse::<f32>() { *upper_outlier_fraction = v; } } if nested_name == "method.averaging_method" { *averaging_method = match value { "Mean" => SegmentationThresholdAveragingSettings::Mean, "Median" => SegmentationThresholdAveragingSettings::Median, _ => (*averaging_method).clone() }; } if nested_name == "method.deviations_above_average" { if let Ok(v) = value.parse::<f32>() { *deviations_above_average = v; } } }
                                ...
                                if nested_name == "value_source" {
                                    item.value_source = match value { "Actual Image" => SegmentationThresholdThresholdValueSourceSettings::ActualImage, "Raw Image" => SegmentationThresholdThresholdValueSourceSettings::RawImage, "Memory" => SegmentationThresholdThresholdValueSourceSettings::Memory(MemoryId::default()), _ => (item.value_source).clone() };
                                }
                            }
                        }
                    }
            ...
            Self::TransformObjects(s) => {
                if param_name == "function" {
                    s.function = match value {
                        "Scale" => ObjectTransformObjectsTransformFunctionSettings::Scale {
                            factor: 1.0f32,
                        },
                        "Snap Area" => ObjectTransformObjectsTransformFunctionSettings::SnapArea {
                            extra_size: 0.0f32,
                        ...
                        "Expand" => ObjectTransformObjectsTransformFunctionSettings::Expand {
                            margin: 0.0f32,
                            unit: SizeUnits::NanoMeter,
                        },
                        "Shrink" => ObjectTransformObjectsTransformFunctionSettings::Shrink {
                            margin: 0.0f32,
                            unit: SizeUnits::NanoMeter,
                        },
                        _ => (s.function).clone(),
                ...
                if let ObjectTransformObjectsTransformFunctionSettings::Scale { ref mut factor } =
                    s.function
                {
                ...
                if let ObjectTransformObjectsTransformFunctionSettings::SnapArea {
                    ref mut extra_size,
                    ref mut unit,
                } = s.function
                {
                ...
                if let ObjectTransformObjectsTransformFunctionSettings::MinCircle {
                ...
                if let ObjectTransformObjectsTransformFunctionSettings::DrawCircle {
                    ref mut diameter,
                    ref mut unit,
                } = s.function
                {
                ...
                if let ObjectTransformObjectsTransformFunctionSettings::FittingEllipse {
                    ref mut scale,
                } = s.function
                {
                ...
                if let ObjectTransformObjectsTransformFunctionSettings::Expand {
                    ref mut margin,
                    ref mut unit,
                } = s.function
                {
                ...
                if let ObjectTransformObjectsTransformFunctionSettings::Shrink {
                    ref mut margin,
                    ref mut unit,
                } = s.function
                {
            ...
            Self::Watershed(s) => {
                ...
                if param_name == "seed_source" {
                    s.seed_source = match value {
                        "Distance Map" => SegmentationWatershedSeedSourceSettings::DistanceMap,
                        "Intensity" => SegmentationWatershedSeedSourceSettings::Intensity,
                        _ => (s.seed_source).clone(),
                    };
                }
            }
