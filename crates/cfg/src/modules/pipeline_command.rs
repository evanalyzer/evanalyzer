    ...
    pub fn to_parameters(&self) -> Vec<ParameterDef> {
        match self {
            ...
            Self::ClassifyRois(_s) => vec![
                ...
                ParameterDef { name: "match_handling".to_string(), display_name: "Match Handling".to_string(), description: "What to do with object class labels after criteria evaluation\n\nControls whether the output class is added or existing classes are removed,\nand whether the action is triggered on a criteria **match** or a **non-match**:\n\n- **AddOutputClassIfMatch** - append the output class to objects that pass the criteria.\n- **AddOutputClassIfNotMatch** - append the output class to objects that fail the criteria.\n- **RemoveInputClassIfMatch / NotMatch** - strip all input classes from matching / non-matching objects.\n- **RemoveOutputClassIfMatch / NotMatch** - strip the output class from matching / non-matching objects.\n- **RemoveAllClassesIfMatch / NotMatch** - clear every class label from matching / non-matching objects.".to_string(), value: match _s.match_handling { ClassificationClassifyRoisClassifyMatchHandlingSettings::AddOutputClassIfMatch => "If match add output class".to_string(), ClassificationClassifyRoisClassifyMatchHandlingSettings::AddOutputClassIfNotMatch => "If not match add output class".to_string(), ClassificationClassifyRoisClassifyMatchHandlingSettings::RemoveInputClassIfMatch => "If match remove input class".to_string(), ClassificationClassifyRoisClassifyMatchHandlingSettings::RemoveInputClassIfNotMatch => "If not match remove input class".to_string(), ClassificationClassifyRoisClassifyMatchHandlingSettings::RemoveOutputClassIfMatch => "If match remove output class".to_string(), ClassificationClassifyRoisClassifyMatchHandlingSettings::RemoveOutputClassIfNotMatch => "If not match remove output class".to_string(), ClassificationClassifyRoisClassifyMatchHandlingSettings::RemoveAllClassesIfMatch => "If match remove all classes".to_string(), ClassificationClassifyRoisClassifyMatchHandlingSettings::RemoveAllClassesIfNotMatch => "If not match remove all classes".to_string() }, param_type: ParamType::Dropdown, options: vec!["If match add output class".to_string(), "If not match add output class".to_string(), "If match remove input class".to_string(), "If not match remove input class".to_string(), "If match remove output class".to_string(), "If not match remove output class".to_string(), "If match remove all classes".to_string(), "If not match remove all classes".to_string()], min: 0.0f32, max: 0.0f32, step: 1.0000f32, groups: vec![] },
        ...
        match self {
            ...
            Self::ClassifyRois(s) => {
                ...
                if param_name == "match_handling" {
                    s.match_handling = match value { "If match add output class" => ClassificationClassifyRoisClassifyMatchHandlingSettings::AddOutputClassIfMatch, "If not match add output class" => ClassificationClassifyRoisClassifyMatchHandlingSettings::AddOutputClassIfNotMatch, "If match remove input class" => ClassificationClassifyRoisClassifyMatchHandlingSettings::RemoveInputClassIfMatch, "If not match remove input class" => ClassificationClassifyRoisClassifyMatchHandlingSettings::RemoveInputClassIfNotMatch, "If match remove output class" => ClassificationClassifyRoisClassifyMatchHandlingSettings::RemoveOutputClassIfMatch, "If not match remove output class" => ClassificationClassifyRoisClassifyMatchHandlingSettings::RemoveOutputClassIfNotMatch, "If match remove all classes" => ClassificationClassifyRoisClassifyMatchHandlingSettings::RemoveAllClassesIfMatch, "If not match remove all classes" => ClassificationClassifyRoisClassifyMatchHandlingSettings::RemoveAllClassesIfNotMatch, _ => s.match_handling.clone() };
                }
