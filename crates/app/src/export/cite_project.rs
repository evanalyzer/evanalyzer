...
pub fn cite_project(config: &ProjectSettings, output_file: &PathBuf) -> Result<(), InternalErrors> {
    ...
    writeln!(out, "# Citations for \"{}\"", config.metadata.name).ok();
    writeln!(out).ok();
...
mod tests {
    ...
    fn writes_ascii_flow_and_citations() {
        ...
        config.metadata.name = "Demo Project".to_string();
        config.pipelines.push(PipelineSettings {
            id: PipelineId(1),
            name: Some("Cell Segmentation & Background Subtraction".to_string()),
