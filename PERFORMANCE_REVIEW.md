...
- [x] **GUI preview clones the entire project on every parameter tweak** — [`pipelines_controller.rs:1091`](crates/gui/src/editor/pipelines_controller.rs#L1091)
  ...
  *Done:* Added `PipelinesController::build_preview_project_settings()`, a pure helper that clones every `ProjectSettings` field except `images.list` and builds that map fresh with only the selected image, instead of cloning the full map (every other project image's `ImageEntry`, including its `SeriesSettings::rois` - full `RoiSettings` with per-ROI mask data) and immediately `.clear()`ing it. `trigger_pipeline_preview_execution` now calls this helper instead of `project.clone()` + `clear()` + `insert()`. Added 3 unit tests (`build_preview_project_settings_keeps_only_the_selected_image`, `_preserves_every_other_project_field`, `_does_not_mutate_the_source_project`). Measured with a synthetic project of 200 images x 50 ROIs each (200x200 mask, 20 calls, release build): 497.7ms -> 659.5μs total, ~755x faster (~24.9ms -> ~33μs per preview trigger) - the gain scales with how much ROI data other images in the project already hold, which is exactly the case that matters (plate-style projects with prior analysis results). Full suites: `evanalyzer_core` 284 passed, `evanalyzer_gui` 92 passed.
...
- [x] CSV/DuckDB exporters re-scan the full ROI set to discover channel/class schema on every `export()` call instead of caching it once per job — [`storage/file.rs:48-61`](crates/core/src/storage/file.rs#L48), [`storage/duckdb.rs:223-244`](crates/core/src/storage/duckdb.rs#L223)
  *Done:* Investigated first. `export()` is called once per (t, z, tile) work unit, not once per whole image, so each call's `roi_cache` is already just that tile's ROIs, not "the full ROI set" - the scan cost is bounded per call, not project-wide. Measured (300 ROIs/call, 5000 calls, release): CSV's Phase 1+2 rescan cost 592.7ms total (~4.4% of that workload's full `export()` time) vs ~106µs if cached - a real but small saving. DuckDB's `compute_coloc_stats` two-pass scan cost 274.4ms total (~1.1% of its `export()` time); a merged single-pass version I wrote for comparison was *slower* (342.3ms) and isn't real "schema caching" anyway - it computes genuine per-tile colocalization statistics, not a cacheable schema. Concluded the rescan itself isn't worth optimizing on its own.
  Investigation surfaced a bigger, related bug instead: `CsvExporter::export()` reopened the output file (`OpenOptions::open()`) on *every* call - thousands of redundant open/append/flush cycles across a job. Fixed by opening the file once in a new `CsvExporter::new()` constructor and keeping the handle (behind a `Mutex`, mirroring `DuckDbExporter`'s `Mutex<Connection>` pattern) for the exporter's lifetime; `header_written` is now tracked as internal state instead of re-checking `Path::exists()` every call. Added `test_csv_export_across_multiple_calls_writes_header_once_and_appends_every_row` (passed against the old per-call-reopen code first, as a correctness safety net through the refactor). Measured with the same 300-ROI/5000-call workload: 13.53s -> 5.59s, ~2.4x faster (includes the schema-rescan saving above). Note: `CsvExporter` isn't currently wired into the live pipeline export path (only `DuckDbExporter`/`MemoryExporter` are, via `job_generator.rs`) - the GUI's CSV export button uses a separate one-shot implementation in `evanalyzer_app`'s `results_exporter.rs`. Fixed anyway since it's public API (`evanalyzer_core::CsvExporter`) with the same per-tile-call contract as `DuckDbExporter`. Full `evanalyzer_core` suite: 285 passed; `evanalyzer_app`/`evanalyzer_gui` build clean.

---

## Low priority / polish

- [x] `MemoryExporter` clones each ROI twice on export (temp `Vec`, then again inside `to_roi_settings()`) — [`storage/memory.rs:24-37`](crates/core/src/storage/memory.rs#L24)
  *Done:* Removed the intermediate `Vec<Roi>` clone — `to_roi_settings()` only needs `&Roi`, so it's now called directly on borrowed values from `cache.roi_cache.values()` via a single `.map().collect()`, still done entirely outside the mutex lock (preserving the original "expensive work off-lock" intent), then moved into the locked `Vec` with `.extend()` instead of clone-and-push. Added `export_moves_every_roi_into_out_rois_without_dropping_or_duplicating_any` and `export_appends_to_existing_out_rois_rather_than_overwriting` tests. Halves the per-ROI clone work on every export call (was: full `Roi` clone incl. mask data, then a second field-by-field clone inside `to_roi_settings()`; now: one clone). Measured with 5000 ROIs (200×200 mask each, 20 export passes, release build): 560ms → 256ms, ~2.2x faster; scales with ROI count and mask size since `mask_data` dominates the clone cost. Full `evanalyzer_core` suite: 284 passed, 0 failed.
